...
class EditAdvertisementSecondScreen extends StatefulWidget {
  static String routeName = './EditAdvertisementSecondScreen';
  ...
  final String tripId;
  final dynamic boatImageList;
  final String couponCode;
  final String startDate;
  final String endDate;
  final String couponDiscount;
  final String discount;
  const EditAdvertisementSecondScreen(
      {super.key,
      this.coverImage,
  ...
  @override
  State<EditAdvertisementSecondScreen> createState() =>
      _AddAdvertisementSecondScreenState();
}

class _AddAdvertisementSecondScreenState
    extends State<EditAdvertisementSecondScreen> {
  ...
  List<String> _selectedDateStrings = [];
  final DateFormat _dateFormat = DateFormat('yyyy-MM-dd');
  List<List<dynamic>> fetchedData = [];
  List<dynamic> addOnList = [];
  List<int> toCheckBox = [];
  bool isApiCalling = false;
  Map<int, Map<int, TextEditingController>> controllersMap =
      {}; // key = addon_id
  ...
  fillData() {
    tripTime = widget.tripDetails["trip_time"] ?? "";
    if (tripTime == 0) {
      _timeController.text =
          "${widget.tripDetails["from_time"]} - ${widget.tripDetails["to_time"]}";
      sendStartTime = widget.tripDetails["from_time"] ?? "";
      sendEndTime = widget.tripDetails["to_time"] ?? "";
    } else {
      _timeController.text = "${widget.tripDetails["from_time"]}";
      sendStartTime = widget.tripDetails["from_time"] ?? "";
    }
    tripDate = widget.tripDetails["trip_date"] ?? "";
    _selectedDateStrings = widget.tripDetails["dates"] != null
        ? widget.tripDetails["dates"].split(', ')
        : [];
    slotPriceTextEditingController.text =
        widget.tripDetails["price_per_hour"].toString();
    minimumHoursTextEditingController.text =
        widget.tripDetails["minimum_hours"].toString();
    idleHoursTextEditingController.text =
        widget.tripDetails["idle_hours"].toString();
    reetocancelbeforeTextEditingController.text =
        widget.tripDetails["free_to_cancel"].toString();
    setState(() {});
  }
  ...
  void initializeControllers() {
    for (var addon in addOnList) {
      ...
      controllersMap[addonId] = {};

      for (var sub in addon['subcategories']) {
        int subId = sub['addon_subcategory_id'];
        controllersMap[addonId]![subId] = TextEditingController();
      }
    }
  }
      ...
      if (response.statusCode == 200) {
        ...
        if (res['success'] == true) {
          ...
          Future.delayed(const Duration(seconds: 1),
              () => prefillControllersFromApi(widget.tripDetails["addons"]));
          setState(() {
            isApiCalling = false;
          });
        } else {
  ...
  void prefillControllersFromApi(List<dynamic> fetchedAddons) {
    log("runnn");
    for (var addon in fetchedAddons) {
      int addonId = addon['addon_id'];
      int subId = addon['addon_subcategory_id'];
      String price = addon['price'].toString();

      if (controllersMap[addonId] != null &&
          controllersMap[addonId]![subId] != null) {
        controllersMap[addonId]![subId]!.text = price;
        if (!toCheckBox.contains(subId)) toCheckBox.add(subId);
      }
    }

    fetchAllPrices(); // To sync selectedAddOns if needed
  }
  ...
  //------------------------Edit advertisement API CALL--------------------------------//
  editAdvertisementApiCall() async {
    setState(() {
      isApiCalling = true;
    });

    Uri url = Uri.parse("${AppConfigProvider.apiUrl}edit_trip");
    ...
    try {
      ...
      List data1 = widget.boatImageList;
      List<File> imageListdata = [];
      if (widget.boatImageList.isNotEmpty) {
        for (var i = 0; i < data1.length; i++) {
          print(data1[i]);
          if (data1[i]['trip_image_id'] == 0) {
            imageListdata.add(data1[i]['image']);
          }
        }
      ...
      log("response417--==> ${formData.fields}");
                    ...
                    child: Column(
                      children: [
                                            ...
                                            decoration: InputDecoration(
                                              ...
                                              suffixIcon: Padding(
                                                ...
                                                child: SizedBox(
                                                  ...
                                                  child: Image.asset(
                                                    AppImage
                                                        .calanderIcon, // Replace with your time icon
                                                    height: 1,
                                                  ),
                                                ),
                                              ),
                                              ...
                                              if (pickedStart != null) {
                                                ...
                                                setState(() {
                                                  ...
                                                  sendStartTime =
                                                      startFormatted;
                                                  selectedStartTime =
                                                      fixedStart; // Optional: store the fixed time
                                                });
                                              }
                                            },
                                            decoration: InputDecoration(
                        ...
                        CustomTextFormFieldBlackWidth(
                          ...
                          hintText:
                              "${AppLanguage.minimumHoursText[language]}*",
                          keyboardtype: TextInputType.number,
                          maxLength: 2,
                          fillColorStatus: 0,
                          readOnly: false,
                            ...
                            children: [
                              TableCalendar(
                                ...
                                firstDay: DateTime.utc(
                                    2020, 1, 1), // Keep the full visible range
                                lastDay: DateTime.utc(2030, 12, 31),
                                availableGestures: AvailableGestures.none,
                                selectedDayPredicate: (day) {
                                  final dateStr = _dateFormat.format(day);
                                  return _selectedDateStrings
                                      .contains(dateStr.trim());
                                },
                                enabledDayPredicate: (day) {
                                  // Allow only today and future dates to be selectable
                                ...
                                onDaySelected: (selectedDay, focusedDay) {
                                  ...
                                  setState(() {
                                    ...
                                    // Toggle logic
                                    if (_selectedDateStrings
                                        .contains(dateStr.trim())) {
                                      _selectedDateStrings
                                          .remove(dateStr.trim());
                                    } else {
                                      _selectedDateStrings.add(dateStr.trim());
                                    }
                                  });
                                },
                                ...
                                calendarStyle: CalendarStyle(
                                  ...
                                  disabledTextStyle: const TextStyle(
                                      color: Colors
                                          .grey), // Optional: style for disabled dates
                                ),
                              ),
                          ...
                          child: Container(
                            ...
                            child: Row(
                              children: [
                                ...
                                Text(
                                  AppLanguage
                                      .weekendFridaySaturdayText[language],
                                  style: const TextStyle(
                                      color: AppColor.primaryColor,
                                      fontSize: 16,
                                      fontWeight: FontWeight.w400,
                        ...
                        //   width: MediaQuery.of(context).size.width * 90 / 100,
                        //   child: Text(
                        //     AppLanguage.equipmentsText[language],
                        //     style: const TextStyle(
                        //         color: AppColor.primaryColor,
                        //         fontSize: 20,
                        //         fontWeight: FontWeight.w700,
                                ...
                                children: [
                                  ...
                                  Wrap(
                                    ...
                                    children: List.generate(
                                        subcategories.length, (subIndex) {
                                      var sub = subcategories[subIndex];
                                      int subId = sub['addon_subcategory_id'];

                                      // Get controller from the new map structure
                                      var controller = controllersMap[addonId]
                                              ?[subId] ??
                                          TextEditingController();
                        ...
                        //     children: [
                        //       Customfild(
                        //           AppLanguage.maskText[language],
                        //           mask1TextEditingController,
                        //           mask2TextEditingController,
                        //           1),
                        //       Customfild(
                        //           AppLanguage.shotgunText[language],
                        //           mask1TextEditingController,
                        //           mask2TextEditingController,
                        //           2)
                        //     ],
                        ...
                        //     children: [
                        //       Customfild(
                        //           AppLanguage.fullgearText[language],
                        //           fullgear1TextEditingController,
                        //           fullgear2TextEditingController,
                        //           3),
                        //       Customfild(
                        //           AppLanguage.snorkelgearText[language],
                        //           snorkelgear1TextEditingController,
                        //           snorkelgear2TextEditingController,
                        //           4)
                        //     ],
                        //   ),
                        // ),
                        ...
                        //     children: [
                        //       Customfild(
                        //           AppLanguage.fishingrodText[language],
                        //           fishingrod1TextEditingController,
                        //           fishingrod2TextEditingController,
                        //           5),
                        //       Customfild(
                        //           AppLanguage.seathreadText[language],
                        //           seathread1TextEditingController,
                        //           seathread2TextEditingController,
                        //           6)
                        //     ],
                        //   ),
                        // ),
                        ...
                        //     children: [
                        //       Customfild(
                        //           AppLanguage.snacksText[language],
                        //           snacks1TextEditingController,
                        //           snacks2TextEditingController,
                        //           7),
                        //       Customfild(
                        //           AppLanguage.beveragesText[language],
                        //           beverages1TextEditingController,
                        //           beverages2TextEditingController,
                        //           8)
                        //     ],
                        //   ),
                        // ),
                        ...
                        //       Customfild(
                        //           AppLanguage.snacksText[language],
                        //           snackssecond1TextEditingController,
                        //           snackssecond2TextEditingController,
                        //           9),
                        //       Customfild(
                        //           AppLanguage.vipCatringText[language],
                        //           vipCatring1TextEditingController,
                        //           vipCatring2TextEditingController,
                        //           10)
                        //     ],
                        //   ),
                        // ),
                        ...
                        //   width: MediaQuery.of(context).size.width * 90 / 100,
                        //   child: Text(
                        //     AppLanguage.entertainmentText[language],
                        //     style: const TextStyle(
                        //         color: AppColor.primaryColor,
                        //         fontSize: 20,
                        //         fontWeight: FontWeight.w700,
                        ...
                        //     children: [
                        //       Customfild(
                        //           AppLanguage.donuteText[language],
                        //           donute1TextEditingController,
                        //           donute2TextEditingController,
                        //           11),
                        //       Customfild(
                        //           AppLanguage.bananaText[language],
                        //           banana1TextEditingController,
                        //           banana2TextEditingController,
                        //           12)
                        //     ],
                        //   ),
                        // ),
                        ...
                        //     children: [
                        //       Customfild(
                        //           AppLanguage.watercarpetText[language],
                        //           watercarpet1TextEditingController,
                        //           watercarpet2TextEditingController,
                        //           13),
                        //       Customfild(
                        //           AppLanguage.dowaniyaText[language],
                        //           dowaniya1TextEditingController,
                        //           dowaniya2TextEditingController,
                        //           14)
                        //     ],
                        //   ),
                        // ),
                        ...
                        //   width: MediaQuery.of(context).size.width * 90 / 100,
                        //   child: Customfild(
                        //       AppLanguage.inflatableBoardText[language],
                        //       inflatableBoard1TextEditingController,
                        //       inflatableBoard2TextEditingController,
                        //       15),
                        // ),
                        // SizedBox(
                        //   height: MediaQuery.of(context).size.height * 2 / 100,
                        ...
                        //   height: MediaQuery.of(context).size.height * 2 / 100,
                        // ),
                        // //! Om AIM aradim island and Garouh Island
                        // Container(
                        //   width: MediaQuery.of(context).size.width * 90 / 100,
                        //   child: Row(
                        //     mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        //     children: [
                        //       Customfild(
                        //           AppLanguage.omAIMaradimislandText[language],
                        //           omAIMaradimisland1TextEditingController,
                        //           omAIMaradimisland2TextEditingController,
                        //           16),
                        //       Customfild(
                        //           AppLanguage.garouhIslandText[language],
                        //           garouhIsland1TextEditingController,
                        //           garouhIsland2TextEditingController,
                        //           17)
                        //     ],
                        //   ),
                        // ),
                        ...
                        //   height: MediaQuery.of(context).size.height * 2 / 100,
                        // ),
                        // //! Failka island and Kubbar Island
                        // Container(
                        //   width: MediaQuery.of(context).size.width * 90 / 100,
                        //   child: Row(
                        //     mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        //     children: [
                        //       Customfild(
                        //           AppLanguage.failkaislandText[language],
                        //           failkaisland1TextEditingController,
                        //           failkaisland2TextEditingController,
                        //           18),
                        //       Customfild(
                        //           AppLanguage.kubbarIslandText[language],
                        //           kubbarIsland1TextEditingController,
                        //           kubbarIsland2TextEditingController,
                        //           19)
                        //     ],
                        //   ),
                        // ),
                        ...
                        //   width: MediaQuery.of(context).size.width * 90 / 100,
                        //   child: Customfild(
                        //       AppLanguage.ouhahIslandText[language],
                        //       ouhahIsland1TextEditingController,
                        //       ouhahIsland2TextEditingController,
                        //       20),
                        // ),
                        // SizedBox(
                        //   height: MediaQuery.of(context).size.height * 2 / 100,
  ...
  void fetchAllPrices() {
    for (var addonEntry in controllersMap.entries) {
      final addonId = addonEntry.key;
      final subMap =
          addonEntry.value; // Map<addon_subcategory_id, TextEditingController>

      for (var subEntry in subMap.entries) {
        final subcategoryId = subEntry.key;
        final controller = subEntry.value;
        final priceText = controller.text.trim();

        if (priceText.isEmpty) continue; // Skip empty
        ...
        if (existingIndex != -1) {
          selectedAddOns[existingIndex] = newEntry; // update
        } else {
          selectedAddOns.add(newEntry); // add
        }
    ...
    // Remove any entries with empty price (optional cleanup)
    selectedAddOns.removeWhere((element) {
